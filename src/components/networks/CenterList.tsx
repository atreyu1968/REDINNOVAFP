...
const CenterList: React.FC<CenterListProps> = ({ onEdit }) => {
  const { centers, deleteCenter, removeDuplicates } = useNetworkStore();
  const { showNotification } = useNotifications();
  const [selectedIsland, setSelectedIsland] = useState<Island | ''>('');
  const [selectedType, setSelectedType] = useState<'CIFP' | 'IES' | ''>('');
  ...
  const filteredCenters = centers.filter((center) => {
    ...
    if (selectedType && center.type !== selectedType) return false;
    return true;
  });
  ...
  const downloadTemplate = () => {
    const headers = ['codigo,nombre,tipo,isla,subred_id'];
    const exampleData = ['IES001,IES Example,IES,Tenerife,SUBNET001'];
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div className="flex space-x-4">
          ...
          <select
            value={selectedType}
            onChange={(e) => setSelectedType(e.target.value as 'CIFP' | 'IES' | '')}
            className="rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
          >
