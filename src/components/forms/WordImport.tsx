...
const WordImport: React.FC<WordImportProps> = ({ onImport, onClose }) => {
  ...
  const processWordDocument = async (file: File): Promise<FormField[]> => {
    ...
    return sections.map((section): FormField => {
      const [title, ...content] = section.trim().split('\n');
      const contentText = content.join('\n').trim();

      // Si la sección parece una pregunta, crear un campo de respuesta
      if (title.endsWith('?')) {
        return {
      ...
      // Si la sección tiene opciones numeradas o con viñetas, crear un campo de selección
      ...
      // Por defecto, crear una sección con un campo de texto
      return {
        id: crypto.randomUUID(),
        type: 'section',
        label: title.replace(/:$/, ''),
